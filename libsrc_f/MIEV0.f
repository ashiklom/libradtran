...
c  CDENAN,         (DOUBLE COMPLEX) denominators of An,Bn
c   CDENBN
...
c                     ( DOUBLE PRECISION version, for when imag refrac index = 0 )

c  RIORIV          1 / Mre

c  RN              1 / N

c  RTMP            (DOUBLE PRECISION) temporary variable
...
c     .. Local Scalars ..
      ...
      REAL      RATIO, DENAN, DENBN
      DOUBLE PRECISION CHIN, CHINM1, COEFF, FN, MIM, MM, 
     &          MRE, NP1DN, PSIN, PSINM1, RIORIV, RN, RTMP, TAUN,
     &          TCOEF, TWONP1, XINV
      DOUBLE COMPLEX   AN, ANM1, ANP, ANPM, BN, BNM1, BNP, BNPM, CDENAN,
     &          CDENBN, CIOR, CIORIV, CTMP, ZET, DSFORW, DSBACK, 
     &          ZETN, ZETNM1, CSUM1, CSUM2
c     ..
c     .. Local Arrays ..

      LOGICAL   CALCMO( 4 )
      DOUBLE PRECISION   PIN( MAXANG ), PINM1( MAXANG ), RBIGA( MAXTRM )
      DOUBLE COMPLEX   CBIGA( MAXTRM ), LITA( MAXTRM ), LITB( MAXTRM ),
     &          SM( MAXANG ), SMS( MXANG2 ), SP( MAXANG ), SPS( MXANG2 )
c     ..
c     .. External Subroutines ..

      EXTERNAL  BIGA, CKINMI, ERRMSG, LPCOEF, MIPRNT, SMALL1, 
     &          SMALL2, TESTMI
c     ..
c     .. Intrinsic Functions ..

      INTRINSIC ABS, DIMAG, DCMPLX, DCONJG, DCOS, MAX, MIN, CMPLX,
     &          DSIN, DREAL, CONJG, AIMAG, REAL
c     ..
      SAVE      PASS1
...
c                                            ** small-particle limit
     ...
     &                S1, S2, TFORW, TBACK, LITA, LITB )
      ...
      IF( .NOT.PERFCT ) THEN
         ...
         MRE    = DREAL( CIOR )
         MIM    = -DIMAG( CIOR )
         NOABS  = REAL(MIM).LE.MIMCUT
         CIORIV = 1.D0 / CIOR
         RIORIV = 1.D0 / MRE

         IF( REAL(XX)*MAX( 1.0, ABS(CREFIN) ).LE.0.1 ) THEN

c                                    ** Use general-refractive-index
c                                    ** small-particle limit

            CALL SMALL2( XX, CIOR, REAL(MIM).GT.MIMCUT, NUMANG, XMU,
     &                   QEXT, QSCA, GQSC, SFORW, SBACK, S1, S2, 
     &                   TFORW, TBACK, LITA, LITB )
...
c                            ** J-Bessel-fcn., A-sub-(1 to NTrm)
      ...
      IF( .NOT.PERFCT ) CALL BIGA( CIOR, XX, NTRM, NOABS, YESANG, 
     &                              RBIGA, CBIGA )
...
c                           ** Compute various numerical coefficients
         ...
         NP1DN  = 1.D0 + RN
         TWONP1 = 2D0*FN + 1.D0
         COEFF  = TWONP1 / ( FN * ( FN + 1.D0 ) )
         TCOEF  = TWONP1 * ( FN * ( FN + 1.D0 ) )
...
c                                         ** Eq (R7)
            ...
            QSCA   = QSCA + REAL(TWONP1*( SQ( AN ) + SQ( BN ) ))
...
c                                               ** Flag resonance spikes
            DENAN  = REAL(ABS( CDENAN ))
            DENBN  = REAL(ABS( CDENBN ))
c                                                   ** Eq. R/B.9
            RATIO  = DENAN / DENBN
c                                                   ** Eq. R/B.10
...
c                                                   ** Eq. R/B.2
         DSFORW = DSFORW + TWONP1*( AN + BN )
c                                                   ** Eq. R/B.5,6
         CSUM1 = CSUM1 + TCOEF *( AN - BN ) 
c                                                   ** Eq. R/B.1
         DSBACK = DSBACK + ( MM*TWONP1 )*( AN - BN ) 
c                                                   ** Eq. R/B.7,8
         CSUM2 = CSUM2 + ( MM*TCOEF ) *( AN + BN ) 

c                                         ** Eq (R8)

         GQSC  = GQSC  + REAL(( FN - RN ) * DREAL( ANM1 * DCONJG( AN ) +
     &                                       BNM1 * DCONJG( BN ) )
...
c                                         ** Eq (R6)
      QEXT = REAL(2.D0 / XX**2 * DREAL(DSFORW))
      ...
      ELSE
         ...
         QSCA = 2.0/ REAL(XX)**2 * QSCA

      END IF

      GQSC   = 4./ REAL(XX)**2 * GQSC
      SFORW  = 0.5*CMPLX(DSFORW)
      SBACK  = 0.5*CMPLX(DSBACK)
      TFORW( 1 ) =  0.5*SFORW - 0.125*CMPLX(CSUM1) 
      TFORW( 2 ) =  0.5*SFORW + 0.125*CMPLX(CSUM1)
      TBACK( 1 ) = -0.5*SBACK + 0.125*CMPLX(CSUM2)
      TBACK( 2 ) =  0.5*SBACK + 0.125*CMPLX(CSUM2)
...
c                                         ** vectorizable loop
            DO 70 J = 1, NUMANG
c                                                   ** Eq (R11)
               S1( J ) = 0.5*CMPLX( SP( J ) + SM( J ) )
               S2( J ) = 0.5*CMPLX( SP( J ) - SM( J ) )
   70       CONTINUE
...
c                                         ** vectorizable loop
            DO 90 J = 1, NANGD2
               S1( NUMANG + 1 - J ) = 0.5*CMPLX( SPS( J ) + SMS( J ) )
               S2( NUMANG + 1 - J ) = 0.5*CMPLX( SPS( J ) - SMS( J ) )
   90       CONTINUE
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC ABS, DIMAG, ICHAR, MAX, DREAL, REAL
c     ..
      ...
      IF( NUMANG.GT.MAXANG ) INPERR = WRTDIM( quiet, 'MaxAng', NUMANG )
      IF( NUMANG.LT.0 ) INPERR = WRTBAD( quiet, 'NUMANG' )

      IF( XX.LT.0.D0 ) INPERR = WRTBAD( quiet, 'XX' )

      IF( .NOT.PERFCT .AND. REAL( CREFIN ).LE.0. )
     &    INPERR = WRTBAD( quiet, 'CREFIN' )

      IF( MOMDIM.LT.0 ) INPERR = WRTBAD( quiet, 'MOMDIM' )

      IF( NMOM.NE.0 ) THEN

         IF( NMOM.LT.0 .OR. NMOM.GT.MOMDIM ) 
     $        INPERR = WRTBAD( quiet, 'NMOM' )

         IF( ABS( IPOLZN ).GT.4444 ) INPERR = WRTBAD( quiet, 'IPOLZN' )
...
c                                ** computation of cosine
   ...
   40    CONTINUE
     ...
     &                          .TRUE. )
      ...
      IF( XX.GT.20000.D0 .OR. REAL( CREFIN ).GT.10.0 .OR.
...
c     .. Parameters ..
      ...
      PARAMETER ( MAXTRM = 100100, MAXMOM = 2*MAXTRM, MXMOM2 = MAXMOM/2,
     &            MAXRCP = 4*MAXTRM + 2 )
c     ..
c     .. Scalar Arguments ..
...
c     .. Local Arrays ..
      ...
      DOUBLE PRECISION AM( 0:MAXTRM ), BI( 0:MXMOM2 ), 
     &                 BIDEL( 0:MXMOM2 ), RECIP( MAXRCP )
      DOUBLE COMPLEX C( MAXTRM ), CM( MAXTRM ), CS( MAXTRM ), 
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DIMAG, DCONJG, MAX, MIN, MOD, DREAL, REAL,DBLE
c     ..
c     .. Equivalences ..
...
c                                           ** vectorizable loop
  ...
  140          CONTINUE
               ...
               PMOM( L, 1 ) = PMOM( L, 1 ) + REAL( BIDEL( I ) * SUM )
...
c                                           ** vectorizable loop
  ...
  180          CONTINUE
               ...
               PMOM( L, 2 ) = PMOM( L, 2 ) + REAL( BIDEL( I ) * SUM )
...
c                                           ** vectorizable loop
  ...
  200          CONTINUE
               ...
               PMOM( L, 3 ) = PMOM( L, 3 ) + REAL( BIDEL( I ) * SUM )
...
c                                           ** vectorizable loop
  ...
  220          CONTINUE
               ...
               PMOM( L, 4 ) = PMOM( L, 4 ) + REAL( BIDEL( I ) * SUM )
...
c     .. Local Scalars ..
      ...
      INTEGER   L, NUMMOM
      DOUBLE PRECISION      A1SQ, B1SQ
      DOUBLE COMPLEX   A1B1C, CTMP
c     ..
c     .. Intrinsic Functions ..

      INTRINSIC DIMAG, DCONJG, MIN, DREAL, AIMAG, REAL
c     ..
c     .. Statement Functions ..
...
c     ..
      ...
      IF( IPOLZN.LT.0 ) THEN
         ...
         IF( CALCMO( 1 ) ) PMOM( 0, 1 ) = 2.25*REAL(B1SQ)

         IF( CALCMO( 2 ) ) PMOM( 0, 2 ) = 2.25*REAL(A1SQ)
...
c                             ** Loop over moments
         DO 10  L = 0, NUMMOM
            ...
            IF( IPOLZN.EQ.0 ) THEN
               ...
               IF( L.EQ.0 ) PMOM( L, 1 ) = 1.5*REAL( A1SQ + B1SQ )

               IF( L.EQ.1 ) PMOM( L, 1 ) = 1.5*REAL( A1B1C )

               IF( L.EQ.2 ) PMOM( L, 1 ) = 0.15*REAL( A1SQ + B1SQ )
            ...
            IF( CALCMO( 1 ) ) THEN
               ...
               IF( L.EQ.0 ) PMOM( L, 1 ) = 2.25*REAL( A1SQ + B1SQ /3.D0)

               IF( L.EQ.1 ) PMOM( L, 1 ) = 1.5*REAL( A1B1C )

               IF( L.EQ.2 ) PMOM( L, 1 ) = 0.3*REAL(B1SQ)
            ...
            IF( CALCMO( 2 ) ) THEN
               ...
               IF( L.EQ.0 ) PMOM( L, 2 ) = 2.25*REAL( B1SQ + A1SQ /3.D0)

               IF( L.EQ.1 ) PMOM( L, 2 ) = 1.5*REAL( A1B1C )

               IF( L.EQ.2 ) PMOM( L, 2 ) = 0.3*REAL(A1SQ)
            ...
            IF( CALCMO( 3 ) ) THEN
               ...
               IF( L.EQ.1 ) PMOM( L, 3 ) = 0.75*REAL( A1SQ + B1SQ )
...
c     .. Local Scalars ..
      ...
      INTEGER   L, NUMMOM
      DOUBLE PRECISION      A2SQ, B2SQ, PM1, PM2
      DOUBLE COMPLEX  A2C, B2C, CA, CAC, CAT, CB, CBC, CBT, CG, CH, CTMP
c     ..
c     .. Intrinsic Functions ..
...
c                                   ** Loop over Sekera moments
         ...
         DO 10 L = 0, NUMMOM
            ...
            IF( CALCMO( 1 ) ) THEN
               ...
               IF( L.EQ.0 ) PMOM( L, 1 ) = 0.25 * REAL( SQ( CAT )
     &                                      + (100.D0/3.D0)* B2SQ )

               IF( L.EQ.1 ) PMOM( L, 1 ) = (5./3.)*REAL( CAT*B2C )

               IF( L.EQ.2 ) PMOM( L, 1 ) = (10./3.)*REAL(B2SQ)
            ...
            IF( CALCMO( 2 ) ) THEN
               ...
               IF( L.EQ.0 ) PMOM( L, 2 ) = 0.25 * REAL( SQ( CA )
     &                                      + (100.D0/3.D0) * A2SQ )

               IF( L.EQ.1 ) PMOM( L, 2 ) = (5./3.)*REAL( CA*A2C )

               IF( L.EQ.2 ) PMOM( L, 2 ) = (10./3.)*REAL(A2SQ)
     ...
     &                                     REAL( B(2)*CAC + CAT*A2C )
            ...
            IF( CALCMO( 4 ) ) THEN
               ...
               IF( L.EQ.0 ) PMOM( L, 4 ) = REAL(-0.25D0 * DIMAG( CAT
     &                                   * CAC + (100.D0/3.D0)* B(2) 
     &                                   * A2C ) )

               IF( L.EQ.1 ) PMOM( L, 4 ) = REAL( -5.D0/ 6.D0*
     &                                     DIMAG( B(2)*CAC + CAT*A2C ) )

               IF( L.EQ.2 ) PMOM( L, 4 ) = REAL( -10.D0/ 3.D0 
     &                                            * DIMAG( B(2)*A2C ) )
...
c                               ** Loop over Mueller moments
     ...
     &                                 + (200.D0/7.D0) * B2C * A(2) )
               ...
               IF( L.EQ.3 ) PMOM( L, 3 ) = REAL( CH ) / 14.D0
            ...
            IF( CALCMO( 4 ) ) THEN
               ...
               IF( L.EQ.0 ) PMOM( L, 4 ) = REAL(0.25D0 * DIMAG( CAC*CAT
     &                                   + CG + 20.D0* B2C * A(2) ) )

               IF( L.EQ.1 ) PMOM( L, 4 ) = REAL(DIMAG( CAC*CBT + CBC*CAT
     &                                           + 3.D0*CH ) / 12.D0 )

               IF( L.EQ.2 ) PMOM( L, 4 ) = REAL ( 0.1D0 * DIMAG( CG
     &                                 + (200.D0/7.D0) * B2C * A(2) ) )

               IF( L.EQ.3 ) PMOM( L, 4 ) = REAL( DIMAG( CH ) / 14.D0 )

               IF( L.EQ.4 ) PMOM( L, 4 ) = REAL( 40.D0/63.D0*DIMAG( B2C
     &                                           * A(2)) )
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC ABS, DIMAG, DCOS, EXP, DREAL, DSIN
c     ..
c     .. Statement Functions ..
...
c                                                   ** Eq. R47a
      F3( MRE ) = -15.04D0 + MRE*( 8.42D0 + 16.35D0*MRE )
c     ..
...
c                                  ** calculated by up-recurrence
      ...
      IF( MRE.LT.1.0D0 .OR. MRE.GT.10.0D0 .OR. MIM.GT.10.0D0 ) THEN
...
c                                  ** No-absorption case; Eq (R20,21)
            RTMP = DSIN( MRE*XX )
            RBIGA( 1 ) = - REZINV + RTMP /
     &                   ( RTMP*REZINV - DCOS( MRE*XX ) )

            DO 30 N = 2, NTRM
               RBIGA( N ) = -( N*REZINV ) +
     &                      1.D0 / ( ( N*REZINV ) - RBIGA( N - 1 ) )

c    Old fix that is probably not necessary anymore now, to use it,
c    replace the above expression in the DO 30-loop with the following:
c    CE: Fixed instability which caused NAN values (11.01.2011)    
c               NENNER=( N*REZINV ) - RBIGA( N - 1 )
c               IF (ABS(NENNER) .LT. 1E-8) THEN
c                  NENNER = 1E-8
c               END IF
c               RBIGA( N ) = -( N*REZINV ) +
c     &                      1.0 / NENNER
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC ABS, DIMAG, DREAL, DBLE
c     ..
...
c                                  ** Check for convergence; Eq. R31
         ...
         IF (      ABS( DREAL (CAPT) - 1.D0 ).GE.EPS2
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DIMAG, DCONJG, DREAL, REAL, AIMAG, CONJG
c     ..
     ...
     &    'Refractive Index:  Real ', REAL( CREFIN ), 
...
c     .. Parameters ..
      ...
      DOUBLE PRECISION      TWOTHR, FIVTHR, FIVNIN
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DIMAG, DCMPLX, DCONJG, DREAL, CMPLX
c     ..
c     .. Statement Functions ..
...
c                                                       ** Eq. R/A.9
      QSCA = REAL(6.D0* XX**4 *( SQ( A(1) ) + SQ( B(1) ) +
     &           FIVTHR*( SQ( A(2) ) + SQ( B(2) ) ) ) )
      QEXT = QSCA
c                                                       ** Eq. R/A.10
      GQSC = REAL(6.D0* XX**4 *DREAL( A(1)*DCONJG( A(2) + B(1) ) +
     &         ( B(1) + FIVNIN*A(2) )*DCONJG( B(2) ) ))

      RTMP   = 1.5D0 * XX**3
      SFORW  = CMPLX( RTMP*( A(1) + B(1) + FIVTHR*( A(2) + B(2) ) ) )
      SBACK  = CMPLX( RTMP*( A(1) - B(1) - FIVTHR*( A(2) - B(2) ) ) )
      TFORW( 1 ) = CMPLX( RTMP*( B(1) + FIVTHR*( 2.D0*B(2) - A(2) ) ) )
      TFORW( 2 ) = CMPLX( RTMP*( A(1) + FIVTHR*( 2.D0*A(2) - B(2) ) ) )
      TBACK( 1 ) = CMPLX( RTMP*( B(1) - FIVTHR*( 2.D0*B(2) + A(2) ) ) )
      TBACK( 2 ) = CMPLX( RTMP*( A(1) - FIVTHR*( 2.D0*A(2) + B(2) ) ) )


      DO 10 J = 1, NUMANG
c                                                    ** Eq. R/A.11,12

         S1( J ) = CMPLX( RTMP*( A(1) + B(1)*XMU( J ) +
     &                    FIVTHR*( A(2)*XMU( J ) + 
     &                             B(2)*( 2.D0*XMU( J )**2 - 1.D0) ) ) )
         S2( J ) = CMPLX( RTMP*( B(1) + A(1)*XMU( J ) +
     &                    FIVTHR*( B(2)*XMU( J ) + 
     &                             A(2)*( 2.D0*XMU( J )**2 - 1.D0) ) ) )
   10 CONTINUE
...
c     .. Parameters ..
      ...
      DOUBLE PRECISION      TWOTHR, FIVTHR
      PARAMETER ( TWOTHR = 2.D0/3.D0, FIVTHR = 5.D0/3.D0)
c     ..
c     .. Scalar Arguments ..
...
c                                           ** Eq. R40b
      QEXT = QSCA
      IF( CALCQE ) QEXT = REAL( 6.D0*XX * DREAL( A(1) + B(1) +
     &                          FIVTHR*A(2) ) )
...
c                                           ** Eq. R40c
      ...
      RTMP   = 1.5D0 * XX**3
      SFORW  = CMPLX( RTMP*( A(1) + B(1) + FIVTHR*A(2) ) )
      SBACK  = CMPLX( RTMP*( A(1) - B(1) - FIVTHR*A(2) ) )
      TFORW( 1 ) = CMPLX( RTMP*( B(1) - FIVTHR*A(2) ) )
      TFORW( 2 ) = CMPLX( RTMP*( A(1) + 2.*FIVTHR*A(2) ) )
      TBACK( 1 ) = CMPLX( TFORW(1) )
      TBACK( 2 ) = CMPLX( RTMP*( A(1) - 2.*FIVTHR*A(2) ) )


      DO 10 J = 1, NUMANG
c                                      ** Eq. R40d,e

         S1( J ) = CMPLX( RTMP*( A(1) + ( B(1) + FIVTHR*A(2) )*XMU( J)))
         S2( J ) = CMPLX( RTMP*( B(1) + A(1)*XMU( J ) +
     &                    FIVTHR*A(2)*( 2.*XMU( J )**2 - 1.D0) ) )
   10 CONTINUE
...
c     .. Local Scalars ..
      ...
      REAL   ACCUR, CALC, EXACT, MIMSAV, TESTGQ, TESTQE,
     &                   TESTQS, XMUSAV
      DOUBLE PRECISION XXSAV
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC ABS, DIMAG, DREAL, AIMAG, REAL
c     ..
c     .. Statement Functions ..
...
c                                    ** correct answers and abort if bad
     ...
     &       OK = TSTBAD( 'QEXT', REAL(ABS((QEXT - TESTQE ) / TESTQE )))

         IF( WRONG( QSCA,TESTQS ) )
     &       OK = TSTBAD( 'QSCA', REAL(ABS((QSCA - TESTQS ) / TESTQS )))

         IF( WRONG( GQSC,TESTGQ ) )
     &       OK = TSTBAD( 'GQSC', REAL(ABS((GQSC - TESTGQ ) / TESTGQ )))

         IF( WRONG( REAL( SFORW ),REAL( TESTSF ) ) .OR.
     &       WRONG( AIMAG( SFORW ),AIMAG( TESTSF ) ) )
     &       OK = TSTBAD( 'SFORW', REAL(ABS((SFORW - TESTSF) / TESTSF)))

         IF( WRONG( REAL( SBACK ),REAL( TESTSB ) ) .OR.
     &       WRONG( AIMAG( SBACK ),AIMAG( TESTSB ) ) )
     &       OK = TSTBAD( 'SBACK', REAL(ABS((SBACK - TESTSB) / TESTSB)))

         IF( WRONG( REAL( S1(1) ),REAL( TESTS1 ) ) .OR.
     &       WRONG( AIMAG( S1(1) ),AIMAG( TESTS1 ) ) )
     &       OK = TSTBAD( 'S1', REAL(ABS(( S1(1) - TESTS1 ) / TESTS1 )))

         IF( WRONG( REAL( S2(1) ),REAL( TESTS2 ) ) .OR.
     &       WRONG( AIMAG( S2(1) ),AIMAG( TESTS2 ) ) )
     &       OK = TSTBAD( 'S2', REAL(ABS(( S2(1) - TESTS2 ) / TESTS2 )))
     ...
     &          REAL( ABS( ( TFORW(N) - TESTTF(N) ) / TESTTF(N) ) ) )

            IF( WRONG( REAL( TBACK(N) ),REAL( TESTTB(N) ) ) .OR.
     &          WRONG( AIMAG( TBACK(N) ),
     &          AIMAG( TESTTB(N) ) ) ) OK = TSTBAD( 'TBACK',
     &          REAL( ABS( ( TBACK(N) - TESTTB(N) ) / TESTTB(N) ) ) )
   ...
   10    CONTINUE
     ...
     &           OK =  TSTBAD( 'PMOM',  REAL(ABS((PMOM(M,1)-TESTPM(M)) /
     &                                      TESTPM(M) ) ) )
