      SUBROUTINE SSSI (NLYR, DTAUC, SSALB, NMOM, PMOM, TEMPER, WVNMLO,
...
c     .. local variables
      INTEGER LC, IU, J, K
      DOUBLE PRECISION UTOA, G, ALB, EDIR, MUREL
...
c     .. currently only calculation for nadir because
c     .. reflectivity table allows only one direction at present
      IF (NUMU .GT. 1) THEN
         WRITE ( *, '(/,2A,/)' )  ' ******* ERROR >>>>>>  ',
     $        'only nadir radiance calculation possible'
         STOP
      ENDIF

      IF (UMU(1) .NE. 1.0) THEN
         WRITE ( *, '(/,2A,/)' )  ' ******* ERROR >>>>>>  ',
     $        'only nadir radiance calculation possible'
         WRITE ( *, * )  ' UMU(1) = ', UMU(1)
         STOP
...
c     .. check if reflecting cloud at this layer;
c     .. need to add 1 to lctop because C starts from 0 
c     .. while Fortran starts from 1
               IF (LC .EQ. lctop+1) THEN
                  ALB = ALB + ref * UMU0 / 3.1415926
               ENDIF
