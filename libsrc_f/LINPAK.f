...
c $Header: /home/users/buras/cvs2svn-2.3.0/cvs/libRadtran/libsrc_f/LINPAK.f,v 1.1 2001-01-08 09:27:21 arve Exp $
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      SUBROUTINE SGBCO2( ABD, LDA, N, ML, MU, IPVT, RCOND, Z )
...
c     .. External Functions ..
      ...
      REAL      SASUM2, SDOT2
      EXTERNAL  SASUM2, SDOT2
c     ..
c     .. External Subroutines ..

      EXTERNAL  SAXPY2, SGBFA2, SSCAL2
c     ..
...
c                       ** compute 1-norm of A
      ...
      DO 10 J = 1, N
         ...
         ANORM  = MAX( ANORM, SASUM2( L,ABD( IS,J ),1 ) )
...
c                                               ** factor
      ...
      CALL SGBFA2( ABD, LDA, N, ML, MU, IPVT, INFO )
...
c                         ** solve trans(L)*Y = W
      DO 60 KB = 1, N
         ...
         IF( K.LT.N )
     &       Z( K ) = Z( K ) + SDOT2(LM, ABD( M+1, K ), 1, Z( K+1 ), 1 )
...
c                         ** solve L*V = Y
      DO 70 K = 1, N
         ...
         IF( K.LT.N )
     &       CALL SAXPY2( LM, T, ABD( M+1, K ), 1, Z( K+1 ), 1 )
...
c                           ** solve  U*Z = W
      DO 80 KB = 1, N
         ...
         CALL SAXPY2( LM, T, ABD( LA,K ), 1, Z( LZ ), 1 )
...
c                              ** make znorm = 1.0
      ...
      SUBROUTINE SGBFA2( ABD, LDA, N, ML, MU, IPVT, INFO )
...
c                               ** row elimination with column indexing
            ...
            DO 40 J = KP1, JU
               ...
               CALL SAXPY2( LM, T, ABD( M+1, K ), 1, ABD( MM+1, J ), 1)
   ...
   50 CONTINUE
      ...
      SUBROUTINE SGBSL2( ABD, LDA, N, ML, MU, IPVT, B, JOB )
...
c                               ** first solve L*Y = B
         IF( ML.NE.0 ) THEN
            ...
            DO 10 K = 1, NM1
               ...
               CALL SAXPY2( LM, T, ABD( M + 1,K ), 1, B( K + 1 ), 1 )
...
c                           ** now solve  U*X = Y
         DO 20 KB = 1, N
            ...
            CALL SAXPY2( LM, T, ABD( LA,K ), 1, B( LB ), 1 )
...
c                                  ** now solve trans(L)*X = Y
   ...
   40       CONTINUE
      ...
      SUBROUTINE SGECO2( A, LDA, N, IPVT, RCOND, Z )
...
c                        ** compute 1-norm of A
      ANORM  = 0.0E0
      DO 10 J = 1, N
         ANORM  = MAX( ANORM, SASUM2( N,A( 1,J ),1 ) )
   10 CONTINUE
c                                      ** factor

      CALL SGEFA2( A, LDA, N, IPVT, INFO )
...
c                                ** solve trans(L)*Y = W
      DO 60 KB = 1, N
         ...
         IF( K.LT.N )
     &       Z( K ) = Z( K ) + SDOT2(N - K, A( K+1, K ), 1, Z( K+1 ), 1)
...
c                                 ** solve L*V = Y
      ...
      DO 70 K = 1, N
         ...
         IF( K.LT.N ) CALL SAXPY2(N - K, T, A( K + 1,K ), 1, Z( K + 1 ),
     &                            1 )
...
c                                  ** solve  U*Z = V
      ...
      DO 80 KB = 1, N
         ...
         CALL SAXPY2( K - 1, T, A( 1,K ), 1, Z( 1 ), 1 )
...
c                              ** row elimination with column indexing
            DO 10 J = KP1, N
               ...
               CALL SAXPY2( N-K, T, A( K+1, K ), 1, A( K+1, J ), 1 )
   ...
   20 CONTINUE
      ...
      SUBROUTINE SGESL2( A, LDA, N, IPVT, B, JOB )
...
c                                     ** first solve  L*Y = B
         DO 10 K = 1, NM1
            ...
            CALL SAXPY2( N - K, T, A( K+1, K ), 1, B( K+1 ), 1 )
...
c                                    ** now solve  U*X = Y
         DO 20 KB = 1, N
            ...
            CALL SAXPY2( K-1, T, A( 1, K ), 1, B(1), 1 )
...
c                                    ** now solve  trans(l)*x = y
         DO 40 KB = 1, NM1
            ...
            K      = N - KB
            B( K ) = B( K ) + SDOT2( N - K, A( K+1, K ), 1, B( K+1 ), 1)
            L      = IPVT( K )
   ...
   40    CONTINUE
      ...
      REAL FUNCTION SASUM2( N, SX, INCX )
...
c                              ** unroll loop for speed
         DO 30 I = M + 1, N, 6
            SASUM2  = SASUM2 + ABS( SX( I ) ) + ABS( SX( I + 1 ) ) +
   ...
   30    CONTINUE
      ...
      SUBROUTINE SAXPY2( N, SA, SX, INCX, SY, INCY )
...
c               ** nonequal or nonpositive increments.
   ...
   40    CONTINUE
      ...
      REAL FUNCTION SDOT2( N, SX, INCX, SY, INCY )
...
c                              ** unroll loop for speed
         DO 30 I = M + 1, N, 5
            SDOT2 = SDOT2 + SX( I )*SY( I ) + SX( I + 1 )*SY( I + 1 ) +
...
c               ** nonequal or nonpositive increments.
   ...
   40    CONTINUE
      ...
      SUBROUTINE SSCAL2( N, SA, SX, INCX )
...
c                             ** unroll loop for speed
   ...
   30    CONTINUE
      ...
      SUBROUTINE SSWAP2( N, SX, INCX, SY, INCY )
...
c               ** nonequal or nonpositive increments.
   ...
   40    CONTINUE
      ...
      INTEGER FUNCTION ISAMAX2( N, SX, INCX )
