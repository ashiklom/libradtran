...
c                         ** Superficially test if main table messed up
    ...
    1    CONTINUE
      ...
      IF( WAVLEN.LT.WL(1) .OR. WAVLEN.GT.WLT(NWLT) ) THEN
         ...
         WRITE ( 0, '(/,A)' )  
     &        'Error: REFICE--wavelength outside table boundaries'
         WRITE ( 0, '(A,F15.3,A,F5.3,A,F15.3,A,/)' ) 'wavelength: ', 
     &             WAVLEN, ' micro m, lower boundary: ', WL(1), 
     &             ' micro m, upper boundary: ', WLT(NWLT),' micro m'
         REFICE = (0.,0.)
         RETURN
...
c               ** (temperature-dependent case)
         ...
         IF( TEMP.LT.TEMREF(4) .OR. TEMP.GT.TEMREF(1) ) THEN
           ...
           WRITE ( 0, '(/,A)' )  
     &        'Error: REFICE--temperature outside table boundaries'
           WRITE ( 0, '(A,F8.3,A,F8.3,A,F8.3,A,/)' ) 'temperature: ', 
     &             TEMP, 'K, lower boundary: ', TEMREF(4), 
     &             'K, upper boundary: ', TEMREF(1),'K'
            REFICE = (0.,0.)
            RETURN
