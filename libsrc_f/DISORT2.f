c
c Changed as follow by A. Kylling (arve.kylling@nilu.no) 20.12.2000
c
c   * Added the number 2 to all subroutine and function names to 
c     avoid conflicts with disort version 1 variants. Should make it
c     easier to upgrade later I hope.....
c   * Increased MXCLY and MXULV. Who wants to do calculations with 6
c     layers? Also increased MXUMU and MXPHI.
c   * Header stuff is only printed if PRNT(1) is true.
c   * uavgso, uavgdn, and uavgup added as output variables.
c   * added the number 2 to the name of all linpak routines
c   * Changed the limit to calculate xr1 from 0 to 1.e-04
c
c Changed as follow by B. Mayer (bernhard.mayer@dlr.de) 22.02.2001
c
c   * Activated the BDREF option, by adding the RPV 
c     (Rahman, Pinty, Verstraete [1993]) parameters rho0, k, theta 
c     as new input to DISORT2 and passing them on to BDREF()
c   * Added Cox and Munk water parameterization 
c     (taken from Nakashima's RSTAR5); 
c     additional parameters:  btype   BRDF type (1, RPV; 2, Cox and Munk)
c                             bu10    Wind speed (m/s)
c                             bpcl    Pigment concentration (in mg.m-3)
c                             bsal    Salinity (in ppt)
c     also added a variable callnum which stores the number of the DISORT2 
c     call since the start of the uvspec program and passes this value 
c     to the BDREF function.
c
c Changed as follow by R. Buras (robert.buras@dlr.de) 13.07.2009
c
c   * Implemented new functions INTCOR3, SECSCA3
c     they replace INTCOR2 and SECSCA2. Now the intensity corrections
c     are calculated using the phase function, PHASO, defined on cosines
c     of angles, MUP, with a total of NPHAS angles.
c     it is still possible to use the old definition by setting the
c     variable OLDINTCOR to 1

c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
c RCS version control information:
c $Header: /home/users/buras/cvs2svn-2.3.0/cvs/libRadtran/libsrc_f/DISORT2.f,v 1.31 2011-03-01 07:41:25 robert Exp $
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


      SUBROUTINE DISORT2(NLYR, DTAUC, SSALB, NMOM, PMOM, NPHAS, PHASO,
     &                   MUP,
     &                   TEMPER, WVNMLO,
     ...
     &                   btype, brho0, bk, btheta, bsigma, bt1, bt2, 
     &                   bscale, biso, bvol, bgeo, bu10, bpcl, bsal, 
     &                   intcor, oldintcor, quiet )
...
c     .. Scalar Arguments ..
     ...
     &          NMOM, NPHAS, NPHI, NSTR, NTAU, NUMU
      REAL      ACCUR, ALBEDO, BTEMP, FBEAM, FISOT, PHI0, TEMIS, TTEMP,
     &          UMU0, WVNMHI, WVNMLO
      INTEGER   btype
      REAL      brho0, bk, btheta, bsigma, bt1, bt2, bscale
      REAL      biso, bvol, bgeo
      REAL      bu10, bpcl, bsal
      INTEGER   quiet
c     ..
c     .. Array Arguments ..
     ...
     &          UMU( MAXUMU ), UTAU( MAXULV ), U0U( MAXUMU, MAXULV ), 
     &          UU( MAXUMU, MAXULV, MAXPHI ), PHASO( NPHAS, MAXCLY )
      REAL*8    MUP( NPHAS )
...
c     .. Local Scalars ..
      ...
      LOGICAL   COMPAR, CORINT, DELTAM, LYRCUT, PASS1, LNEWINTCOR
      INTEGER   callnum
...
c     .. External Functions ..
      ...
      REAL      PLKAVG2, R1MACH, RATIO2
      EXTERNAL  PLKAVG2, R1MACH, RATIO2
c     ..
c     .. External Subroutines ..

      EXTERNAL  ALBTRN2, CHEKIN2, CMPINT2, FLUXES2, INTCOR2, INTCOR3, 
     &     LEPOLY2, PRAVIN2, PRTINP2, PRTINT2, 
     &     SETDIS2, SETMTX2, SLFTST2, SOLEIG2, SOLVE02,
     &     SURFAC2, TERPEV2, TERPSO2, UPBEAM2, UPISOT2, 
     &     USRINT2, ZEROAL2, ZEROIT2
c     ..
c     .. Intrinsic Functions ..

      INTRINSIC ABS, ASIN, COS, FLOAT, LEN, MAX, SQRT
c     ..
      SAVE      DITHER, PASS1, PI, RPD, SQT, callnum
...
c                            ** Be sure SLFTST sets all print flags off.
         ...
         CALL SLFTST2( CORINT, ACCUR, ALBEDO, BTEMP, DELTAM, DTAUC( 1 ),
...
c                                ** Check input dimensions and variables
      ...
      CALL CHEKIN2(NLYR, DTAUC, SSALB, NMOM, PMOM, TEMPER, WVNMLO,
     ...
     &             btype, brho0, bk, btheta, bsigma, bt1, bt2, 
     &             bscale, biso, bvol, bgeo,
     &             bu10, bpcl, bsal, callnum, quiet )

c                                 ** Zero internal and output arrays

      CALL  ZEROAL2( MXCLY, EXPBEA(1), FLYR, OPRIM, PHASA, PHAST, PHASM,
     &                     TAUCPR(1), XR0, XR1,
...
c                                 ** Perform various setup operations
      ...
      CALL SETDIS2(CMU, CWT, DELTAM, DTAUC, DTAUCP, EXPBEA, FBEAM, FLYR,
...
c                                 ** Print input information
      IF( PRNT( 1 ) )
     &    CALL PRTINP2( NLYR, DTAUC, DTAUCP, SSALB, NMOM, PMOM, TEMPER,
...
c                              ** beam angles at once
      IF( IBCND.EQ.1 ) THEN
         ...
         CALL ALBTRN2( ALBEDO, AMB, APB, ARRAY, B, BDR, CBAND, CC, CMU,
...
c                                   ** Calculate Planck functions
      IF( .NOT.PLANK ) THEN
         ...
         BPLANK = 0.0
         TPLANK = 0.0
         CALL ZEROIT2( PKAG,  MXCLY + 1 )

      ELSE

         TPLANK = TEMIS*PLKAVG2( WVNMLO, WVNMHI, TTEMP )
         BPLANK =       PLKAVG2( WVNMLO, WVNMHI, BTEMP )

         DO 40 LEV = 0, NLYR
            PKAG( LEV ) = PLKAVG2( WVNMLO, WVNMHI, TEMPER( LEV ) )
   40    CONTINUE
...
c                             **     cosines
         IF( FBEAM.GT.0.0 ) THEN
            ...
            CALL LEPOLY2(NCOS, MAZIM, MXCMU, NSTR-1, ANGCOS, SQT, YLM0 )

         END IF


         IF( .NOT.ONLYFL .AND. USRANG )
     &      CALL LEPOLY2(NUMU, MAZIM, MXCMU, NSTR-1, UMU, SQT, YLMU )

         CALL LEPOLY2( NN, MAZIM, MXCMU, NSTR-1, CMU, SQT, YLMC )
...
c                      ** STWL(23f); return eigenvalues and eigenvectors
            ...
            CALL SOLEIG2( AMB, APB, ARRAY, CMU, CWT, GL( 0,LC ), MI,
...
c                                  ** beam source
            IF( FBEAM.GT.0.0 )
     &          CALL UPBEAM2( ARRAY, CC, CMU, DELM0, FBEAM, GL( 0,LC ),
...
c
            IF( PLANK .AND. MAZIM.EQ.0 ) THEN
               ...
               IF( DTAUCP( LC ).GT.1.E-04 ) XR1( LC ) =
     &             ( PKAG( LC ) - PKAG( LC-1 ) ) / DTAUCP( LC )

               XR0( LC ) = PKAG( LC-1 ) - XR1( LC )*TAUCPR( LC-1 )

               CALL UPISOT2( ARRAY, CC, CMU, IPVT, MXCMU, NN, NSTR,
...
c                                            ** to user angles
               ...
               CALL TERPEV2(CWT, EVECC, GL( 0,LC ), GU( 1,1,LC ), MAZIM,
     &                      MXCMU, MXUMU, NN, NSTR, NUMU, WK, YLMC,
     &                      YLMU )
c                                            ** Interpolate source terms
c                                            ** to user angles

               CALL TERPSO2(CWT, DELM0, FBEAM, GL( 0,LC ), MAZIM, MXCMU,
...
c                      ** boundary and layer interface conditions
         ...
         CALL SETMTX2( BDR, CBAND, CMU, CWT, DELM0, DTAUCP, GC, KK,
...
c                      ** geneous solution (general boundary conditions)
         ...
         CALL SOLVE02( B, BDR, BEM, BPLANK, CBAND, CMU, CWT, EXPBEA,
...
c                                  ** Compute upward and downward fluxes
         ...
         IF( MAZIM.EQ.0 )
     &       CALL FLUXES2( CMU, CWT, FBEAM, GC, KK, LAYRU, LL, LYRCUT,
...
c                                     ** at quadrature angles
  ...
  100          CONTINUE
         ...
         CALL ZEROIT2( UUM, MXUMU*MXULV )

         IF( USRANG ) THEN
c                                     ** Compute azimuthal intensity
c                                     ** components at user angles

            CALL USRINT2( BPLANK, CMU, CWT, DELM0, DTAUCP, EMU, EXPBEA,
...
c                                     ** components at quadrature angles
            ...
            CALL CMPINT2(FBEAM, GC, KK, LAYRU, LL, LYRCUT, MAZIM, MXCMU,
...
c                              ** at user angles
            ...
            IF( PRNTU0( 2 ) )
     &          CALL PRAVIN2( UMU, NUMU, MAXUMU, UTAU, NTAU, U0U )

c bug fix RB 28.2.2011
            CALL ZEROIT2( PHIRAD, MXPHI )
            ...
            DO 170 J = 1, NPHI
               ...
               DO 160 LU = 1, NTAU
                  ...
                  DO 150 IU = 1, NUMU
                     ...
                     AZERR  = MAX( AZERR,
     &                        RATIO2( ABS(AZTERM), ABS(UU(IU,LU,J)) ) )
  150             CONTINUE
...
c                                    ** corrections
      ...
      IF( CORINT ) THEN
         IF ( LNEWINTCOR ) THEN
            IF ( QUIET .EQ. 0 .AND. .NOT.PASS1 )
     &         WRITE (0, '(//,2A)')
     &           "Using new intcor, phase functions available"
            CALL INTCOR3( DITHER, FBEAM, FLYR, LAYRU, LYRCUT, MAXMOM,
     &                    MAXULV, MAXUMU, NMOM, NCUT, NPHAS, NPHI, NSTR,
     &                    NTAU, NLYR,
     &                    NUMU, OPRIM, PHASA, PHAST, PHASM, PHIRAD, PI,
     &                    RPD, PMOM, PHASO, MUP, SSALB, DTAUC, TAUC,
     &                    TAUCPR, UMU,
     &                    UMU0, UTAU, UTAUPR, UU )
         ELSE
            IF ( QUIET .EQ. 0 .AND. .NOT.PASS1 )
     &         WRITE (0, '(//,2A)')
     &           "Using old intcor version, no phase available"
            CALL INTCOR2( DITHER, FBEAM, FLYR, LAYRU, LYRCUT, MAXMOM,
     &                    MAXULV, MAXUMU, NMOM, NCUT, NPHI, NSTR,
     &                    NTAU,
     &                    NUMU, OPRIM, PHASA, PHAST, PHASM, PHIRAD, PI,
     &                    RPD, PMOM, SSALB, DTAUC, TAUC,
     &                    TAUCPR, UMU,
     &                    UMU0, UTAU, UTAUPR, UU )
         END IF
      END IF
...
c                                          ** Print intensities
     ...
     &    CALL PRTINT2( UU, UTAU, NTAU, UMU, NUMU, PHI, NPHI, MAXULV,
     &                 MAXUMU )
...
c                                    ** correct answers and abort if bad
     ...
     &                FLUP( 1 ), RFLDIR( 1 ), RFLDN( 1 ), UU( 1,1,1 ) )
      ...
      SUBROUTINE ASYMTX2( AA, EVEC, EVAL, M, IA, IEVEC, IER, WKD, AAD,
     &                   EVECD, EVALD )
...
c     ..
     ...
     &    CALL ERRMSG2( 'ASYMTX--bad input variable(s)', .TRUE. )
...
c                           ** Handle 1x1 and 2x2 special cases
      ...
      ELSE IF( M.EQ.2 ) THEN
         ...
         IF( DISCRI .LT. 0.0 )
     &       CALL ERRMSG2( 'ASYMTX--complex evals in 2x2 case',.TRUE. )
...
c                         ** Put results into output arrays
  ...
  720 CONTINUE
      ...
      SUBROUTINE CMPINT2(FBEAM, GC, KK, LAYRU, LL, LYRCUT, MAZIM, 
...
c                                       ** Loop over user levels
   ...
   40 CONTINUE
      ...
      SUBROUTINE FLUXES2( CMU, CWT, FBEAM, GC, KK, LAYRU, LL, LYRCUT,
     &                   MAXULV, MXCMU, MXULV, NCUT, NN, NSTR, NTAU,
...
c                                        ** Zero DISORT output arrays
      CALL ZEROIT2( U0C, MXULV*MXCMU )
      CALL ZEROIT2( FLDIR, MXULV )
      CALL ZEROIT2( FLDN, MXULV )
...
c                                                ** this level
  ...
  100    CONTINUE
      ...
      SUBROUTINE INTCOR2( DITHER, FBEAM, FLYR, LAYRU, LYRCUT, MAXMOM,
...
c     .. External Functions ..
      ...
      REAL      SECSCA2, SINSCA2
      EXTERNAL  SECSCA2, SINSCA2
c     ..
c     .. Intrinsic Functions ..
   ...
   70       CONTINUE
            ...
            DO 80 LU = 1, NTAU
               ...
               IF( .NOT.LYRCUT .OR. LAYRU( LU ).LT.NCUT ) THEN
                   ...
                   USSNDM  = SINSCA2( DITHER, LAYRU( LU ), NCUT, PHAST,
     &                               SSALB, TAUCPR, UMU( IU ), UMU0,
     &                               UTAUPR( LU ), FBEAM, PI )

                   USSP    = SINSCA2( DITHER, LAYRU( LU ), NCUT, PHASM,
...
c                                ** scattering below top level.
               ...
               DO 90 LU = LTAU, NTAU
                  ...
                  IF( .NOT.LYRCUT .OR. LAYRU( LU ).LT.NCUT ) THEN
                      ...
                      DUIMS = SECSCA2(CTHETA, FLYR, LAYRU( LU ), MAXMOM,
...
c                                ** End loop over zenith angles
  110 CONTINUE
      ...
      REAL FUNCTION SECSCA2( CTHETA, FLYR, LAYRU, MAXMOM, NMOM, NSTR,
...
c                              ** Eq. STWL (A.13)
      ...
      SECSCA2 = FBEAM/( 4.*PI ) * ( FBAR*WBAR )**2 / ( 1.-FBAR*WBAR ) *
     &         PSPIKE * XIFUNC2( -UMU, UMU0P, UMU0P, UTAU )


      RETURN
      END

      SUBROUTINE INTCOR3( DITHER, FBEAM, FLYR, LAYRU, LYRCUT, MAXMOM,
     &                   MAXULV, MAXUMU, NMOM, NCUT, NPHAS, NPHI, NSTR,
     &                   NTAU, NLYR,
     &                   NUMU, OPRIM, PHASA, PHAST, PHASM, PHIRAD, PI,
     &                   RPD, PMOM, PHASO, MUP, SSALB, DTAUC, TAUC,
...
c      IMPLICIT NONE
      ...
      LOGICAL   LYRCUT
      INTEGER   MAXMOM, MAXULV, MAXUMU, NCUT, NMOM, NPHAS, NPHI, NSTR,
     &          NTAU,
     &          NUMU, NLYR
      REAL      DITHER, FBEAM, PI, RPD, UMU0
c     ..
c     .. Array Arguments ..
     ...
     &          UU( MAXUMU, MAXULV, * )
      REAL      PHASO ( NPHAS, * )
      REAL*8    CTHETA, MUP ( NPHAS )
c     ..
c     .. Local Scalars ..

      INTEGER   IU, JP, K, LC, LTAU, LU, IT, LYR
      REAL      DTHETA, DUIMS, THETA0, THETAP,
     &          USSNDM, USSP
c     ..
c     .. External Functions ..

      REAL      SECSCA3, SINSCA2
      INTEGER   LOCATE, DLOCATE
      EXTERNAL  SECSCA3, SINSCA2, LOCATE, DLOCATE
c     ..
c     .. Intrinsic Functions ..
...
c start of BDE stuff

c Initialization of new PSPIKE.
      PARAMETER ( NF = 100 )
      PARAMETER ( ZERO = 1e-4 )
      REAL*8    MU_EQ ( NF, NTAU ), NORM_PHAS ( NTAU ), NORM
      INTEGER   NEG_PHAS ( NF, NTAU )

      REAL*8    PHAS2 ( NPHAS, 1:NTAU ), PHASR ( 1:NLYR )
      REAL*8    F_PHAS2, F_PHAS2_ABS ( NPHAS ), DF
      REAL*8 PL, PLM1, PLM2
      REAL*8 FBAR
      INTEGER I
...
c very likely this can be optimized extremely!!!
c find out whether LAYRU(LU) is monotonic function...
            DO 306 LYR = 1, LAYRU(LU)-1
               PHAS2 ( IT, LU ) = PHAS2 ( IT, LU ) +
     &              ( PHASO ( IT, LYR ) - PHASR ( LYR ) ) *
     &              SSALB ( LYR ) * DTAUC ( LYR )
 306        CONTINUE
 ...
 301  CONTINUE
      ...
      DO 307 LU = 1, NTAU
         ...
         LYR = LAYRU(LU)
         FBAR = FLYR ( LYR ) * SSALB ( LYR ) *
     &        ( UTAU ( LU ) - TAUC ( LYR-1 ) )

         DO 308 LYR = 1, LAYRU(LU)-1
            FBAR = FBAR + SSALB( LYR ) * DTAUC( LYR ) * FLYR( LYR )
 308     CONTINUE

         IF( FBAR.LE.ZERO .OR. FBEAM.LE.ZERO ) THEN
...
c normalize
     ...
     &           + ( MUP(IT) - MUP(IT-1) ) * 0.5 *
     &           ( ABS( PHAS2 ( IT, LU ) ) + ABS( PHAS2 ( IT-1, LU ) ) )
         
 311     CONTINUE

         NORM = 2.0 / F_PHAS2
         DO 312 IT = 1, NPHAS
            PHAS2 ( IT, LU ) = PHAS2 ( IT, LU ) * NORM
 312     CONTINUE
c ab hier nicht mehr so interessant




c prepare integration

         F_PHAS2 = 0.
         DF = F_PHAS2_ABS ( NPHAS ) / ( NF - 1 )
         MU_EQ ( 1, LU ) = -1.
         IF ( PHAS2 ( 1, LU ) .GT. 0. ) THEN
            NEG_PHAS ( 1, LU ) = 0.
         ELSE
            NEG_PHAS ( 1, LU ) = 1.
         END IF
         ...
         DO 313 I = 2, NF-1
            F_PHAS2 = F_PHAS2 + DF
c this can be quicker
            IT = DLOCATE ( NPHAS, F_PHAS2_ABS ( 1 ), F_PHAS2 )
     ...
     &           ( MUP ( IT+1 ) - MUP ( IT ) )
            ...
            IF ( PHAS2 ( IT, LU )   .GT. 0. .AND.
     &           PHAS2 ( IT+1, LU ) .GT. 0. ) THEN
               NEG_PHAS ( I, LU ) = 0.
            ELSE
               IF ( PHAS2 ( IT, LU )   .LT. 0. .AND.
     &              PHAS2 ( IT+1, LU ) .LT. 0. ) THEN
                  NEG_PHAS ( I, LU ) = 1.
               ELSE
                  IF ( PHAS2 ( IT, LU )
     &                 + ( F_PHAS2       - F_PHAS2_ABS ( IT ) ) /
 ...
 313     CONTINUE
         ...
         MU_EQ ( NF, LU ) = 1.
         IF ( PHAS2 ( NPHAS, LU ) .GT. 0. ) THEN
            NEG_PHAS ( NF, LU ) = 0.
         ELSE
            NEG_PHAS ( NF, LU ) = 1.
         END IF

         NORM_PHAS ( LU ) = F_PHAS2_ABS ( NPHAS )
     &                      / ( ( NF - 1 ) * PI ) * NORM
...
c                                ** Interpolate original phase function
c                                ** to actual phase function

            IT = DLOCATE ( NPHAS, MUP ( 1 ), CTHETA )

            DO 20 LC = 1, NCUT

               PHASA( LC ) = PHASO ( IT, LC )
     &                       + ( CTHETA       - MUP ( IT ) ) /
     &                         ( MUP ( IT+1 ) - MUP ( IT ) ) *
     &                         ( PHASO ( IT+1, LC ) - PHASO ( IT, LC ) )
...
c                                ** scattering below top level.
               ...
               DO 90 LU = LTAU, NTAU
                  ...
                  IF( .NOT.LYRCUT .OR. LAYRU( LU ).LT.NCUT ) THEN
                      ...
                      DUIMS = SECSCA3(CTHETA, FLYR, LAYRU( LU ), MAXMOM,
     &                                NMOM, NPHAS, NSTR, NF,
     &                                PMOM, PHASO,
     &                                MUP, PHASA, PHASM,
     &                                PHAS2 ( 1, LU ),
...
c                                ** End loop over zenith angles
  110 CONTINUE
      ...
      REAL FUNCTION SECSCA3( CTHETA, FLYR, LAYRU, MAXMOM, NMOM, NPHAS,
     &                       NSTR, NF,
     &                       PMOM, PHASO, MUP, PHASA, PHASM,
     &                       PHAS2, MU_EQ, NEG_PHAS,
     &                       NORM_PHAS,
     &                       SSALB, DTAUC, TAUC, UMU, UMU0, UTAU,
     &                       FBEAM, PI )
...
c     .. Scalar Arguments ..
      INTEGER   LAYRU, MAXMOM, NMOM, NPHAS, NSTR, NP, NF
      REAL      FBEAM, PI, UMU, UMU0, UTAU
c     ..
c     .. Array Arguments ..
      REAL      DTAUC( * ), FLYR( * ), PMOM( 0:MAXMOM, * ), SSALB( * ),
     &          TAUC( 0:* ),
     &          PHASA( * ), PHASM( * ), PHASO( NMOM, * )
      REAL*8    CTHETA, PHAS2 ( NF ), MU_EQ( NF ),
     &          NORM_PHAS, MUP( NPHAS )
      INTEGER   NEG_PHAS( NF )
c     ..
c     .. Local Scalars ..
      INTEGER   J, K, LYR, IT
      REAL      DTAU, FBAR, GBAR, PL, PLM1, PLM2, PSPIKE, STAU, UMU0P,
     &          WBAR, ZERO, PSPIKE1, PSPIKE2, STHETA, SMUEQ,
     &          PHINT
c     ..
c     .. External Functions ..
      REAL      XIFUNC2
      INTEGER   LOCATE, DLOCATE
      EXTERNAL  XIFUNC2, LOCATE, DLOCATE
c     ..
...
c special case: second scattering angle does not depend on
c azimuth of first scattering angle
         IF ( CTHETA .EQ. 1.0 .OR. MU_EQ ( J ) .EQ. 1.0 ) THEN
            IT = DLOCATE ( NPHAS, MUP ( 1 ), CTHETA * MU_EQ ( J ) )
            PHINT = PI * ( PHAS2 ( IT )
     &           + ( MU_EQ ( J ) * ctheta - MUP ( IT ) )
     &           / ( MUP ( IT+1 ) - MUP ( IT ) )
     &           * ( PHAS2 ( IT+1 ) - PHAS2 ( IT ) ) )
            IF ( CTHETA .EQ. 1.0 ) THEN
               PHINT = PHINT / 2.0
            END IF
         ELSE
            PHINT = 0.0

            SMUEQ = sqrt ( 1. - MU_EQ ( J )**2 )

c locate integration borders
            MUMIN = CTHETA *  MU_EQ ( J ) - STHETA * SMUEQ
            MUMAX = CTHETA *  MU_EQ ( J ) + STHETA * SMUEQ

c cut where mu_1 = mu_2
            IF ( MU_EQ ( J ) .LT. MUMAX ) THEN
               MUMAX = MU_EQ ( J )
               CUTTING=1
            ELSE
               CUTTING=0
            ...
            IF ( MUMIN .LT. MUMAX ) THEN
               IMIN = DLOCATE ( NPHAS, MUP ( 1 ), MUMIN )
               IMAX = DLOCATE ( NPHAS, MUP ( 1 ), MUMAX )

               K=IMIN
c assuming SPF is linear in mu
               D = ( PHAS2 ( K+1 ) - PHAS2 ( K ) )
     &              / ( MUP ( K+1 ) - MUP ( K ) )
               C = PHAS2 ( K ) - MUP ( K ) * D

               PHINT = PHINT +
     &              ( D * ctheta * MU_EQ ( J ) + C ) * PI / 2.0

               DO 322 K = IMIN+1, IMAX

                  DP = ( PHAS2 ( K+1 ) - PHAS2( K ) )
     &                 / ( MUP( K+1 ) - MUP( K ) )
                  CP = PHAS2 ( K ) - MUP ( K ) * DP
     ...
     &                 - MU_EQ ( J ) * MU_EQ ( J )
     &                 + 2.0 * CTHETA * MU_EQ ( J ) * MUP ( K )
     &                 - MUP ( K ) * MUP ( K ) )
     &                 + ( ( DP - D )* CTHETA * MU_EQ ( J ) + CP - C ) *
     &                 ASIN ( ( CTHETA * MU_EQ ( J ) - MUP ( K ) )
     &                 / ( SMUEQ * STHETA ) )
 ...
 322           END DO
               ...
               IF ( CUTTING .EQ. 1 ) THEN
                  PHINT = PHINT
     &                 - D * SQRT ( 1.0 - CTHETA * CTHETA
     &                 + 2.0 * MU_EQ ( J ) * MU_EQ ( J ) *
     &                 ( CTHETA - 1.0 ) )
     &                 - ( D * CTHETA * MU_EQ ( J ) + C ) *
     &                 ASIN ( ( CTHETA - 1.0 ) * MU_EQ ( J )
     &                 / ( SMUEQ * STHETA ) )
               ELSE
                  PHINT = PHINT +
     &                 ( D * CTHETA * MU_EQ ( J ) + C ) * PI / 2.0
         ...
         IF ( J .EQ. 1 .OR. J .EQ. NF ) THEN
            IF ( NEG_PHAS ( J ) .EQ. 1 ) THEN
...
c                              ** Eq. STWL (A.13)
      ...
      SECSCA3 = FBEAM/( 4.*PI ) * ( FBAR*WBAR )**2 / ( 1.-FBAR*WBAR ) *
     &         PSPIKE * XIFUNC2( -UMU, UMU0P, UMU0P, UTAU )
      ...
      SUBROUTINE SETDIS2(CMU, CWT, DELTAM, DTAUC, DTAUCP, EXPBEA, FBEAM,
...
c                               ** Compare beam angle to comput. angles
         DO 90 IQ = 1, NN
            ...
            IF( ABS( (UMU0-CMU(IQ)) / UMU0 ) .LT.1.E-4 ) CALL ERRMSG2(
...
c
     ...
     &   .NOT.DELTAM )  CORINT = .FALSE.
      ...
      SUBROUTINE SETMTX2( BDR, CBAND, CMU, CWT, DELM0, DTAUCP, GC, KK,
     &                   LAMBER, LYRCUT, MI, MI9M2, MXCMU, NCOL, NCUT,
     &                   NNLYRI, NN, NSTR, TAUCPR, WK )
...
c     ..
      ...
      CALL ZEROIT2( CBAND, MI9M2*NNLYRI )
...
c                          ** truncated bottom layer
  ...
  160 CONTINUE
      ...
      REAL FUNCTION  SINSCA2( DITHER, LAYRU, NLYR, PHASE, OMEGA, TAU,
...
c                                 ** UMU=UMU0, Eq. STWL (65e)
         ...
         DO 10 LYR = 1, LAYRU - 1
            SINSCA2 = SINSCA2 + OMEGA( LYR ) * PHASE( LYR ) *
     &               ( TAU( LYR ) - TAU( LYR-1 ) )
   10    CONTINUE

         SINSCA2 = FBEAM / ( 4.*PI * UMU0 ) * EXP0 * ( SINSCA2 +
...
c                                 ** Upward intensity, Eq. STWL (65b)
         DO 20 LYR = LAYRU, NLYR
            ...
            SINSCA2 = SINSCA2 + OMEGA( LYR )*PHASE( LYR )*(EXP0 - EXP1)
            EXP0 = EXP1
...
c                                 ** Downward intensity, Eq. STWL (65d)
   ...
   30    CONTINUE
      ...
      SINSCA2 = FBEAM / ( 4.*PI * ( 1. + UMU/UMU0 ) ) * SINSCA2


      RETURN
      END

      SUBROUTINE SOLEIG2( AMB, APB, ARRAY, CMU, CWT, GL, MI, MAZIM,
...
c                      ** Find (real) eigenvalues and eigenvectors
      ...
      CALL ASYMTX2( ARRAY, EVECC, EVAL, NN, MI, MXCMU, IER, WKD, AAD,
     &             EVECCD, EVALD )

      IF( IER.GT.0 ) THEN

         WRITE( 0, '(//,A,I4,A)' ) ' ASYMTX--eigenvalue no. ',
     &      IER, '  didnt converge.  Lower-numbered eigenvalues wrong.'

         CALL ERRMSG2( 'ASYMTX--convergence problems',.True.)
...
c                                ** reversing sign of 'k' in SS(10) )
  ...
  130 CONTINUE
      ...
      SUBROUTINE SOLVE02( B, BDR, BEM, BPLANK, CBAND, CMU, CWT, EXPBEA,
...
c     .. External Subroutines ..
      ...
      EXTERNAL  ERRMSG2, SGBCO2, SGBSL2, ZEROIT2
c     ..
c     .. Intrinsic Functions ..
...
c                     ** (CBAND is in LINPACK packed format)
      ...
      CALL SGBCO2( CBAND, MI9M2, NCOL, NCD, NCD, IPVT, RCOND, Z )

      IF( 1.0 + RCOND.EQ.1.0 )
     &   CALL ERRMSG2('SOLVE0--SGBCO says matrix near singular',.FALSE.)
...
c                   ** decomposed.  Solution is returned in B.
      ...
      CALL SGBSL2( CBAND, MI9M2, NCOL, NCD, NCD, IPVT, B, 0 )
...
c       NN     :  Order of Double-Gauss quadrature (NSTR/2)
c
c       btype  :  type of surface parameterization
c                   1, RPV
c                   2, Cox and Munk [1954]
c
c       brho0, bk, btheta, bscale:  RPV BRDF parameterization (BM, 23.2.2001)
c
c       bu10   :  Wind speed for Cox and Munk parameterization
c
c       bpcl   :  Pigment concentration for Cox and Munk (in mg.m-3)
c
c       bsal   :  Salinity for Cox and Munk parameterization (in ppt)
c
c
c       (Remainder are 'DISORT' input variables)
c
...
c                             ** at computational angles Eq. STWL (41)
   ...
   30    CONTINUE
     ...
     &                          PI*GMU(K), 
     &                          btype, brho0, bk, btheta, 
     &                          bsigma, bt1, bt2, bscale,
     &                          biso, bvol, bgeo,
     &                          bu10, bpcl, bsal, callnum ) 
     &                   * COS( MAZIM*PI*GMU( K ) )
   40          CONTINUE
...
c                             ** at user angles Eq. STWL (41)
      ...
      IF( .NOT.ONLYFL .AND. USRANG ) THEN
         ...
         CALL ZEROIT2( EMU, MXUMU )
         CALL ZEROIT2( RMU, MXUMU*( MI+1 ) )
...
c                               ** user angle cosines -UMU-.
  ...
  170    CONTINUE
      ...
      SUBROUTINE TERPEV2( CWT, EVECC, GL, GU, MAZIM, MXCMU, MXUMU, NN,
...
c                                    ** and store eigenvectors
   ...
   50 CONTINUE
      ...
      SUBROUTINE TERPSO2( CWT, DELM0, FBEAM, GL, MAZIM, MXCMU, PLANK,
...
c
   ...
   80    CONTINUE
      ...
      SUBROUTINE UPBEAM2( ARRAY, CC, CMU, DELM0, FBEAM, GL, IPVT, MAZIM,
...
c     .. External Subroutines ..
      ...
      EXTERNAL  ERRMSG2, SGECO2, SGESL2
c     ..
...
c                  ** (NOTE:  ARRAY is altered)
      ...
      CALL SGECO2( ARRAY, MXCMU, NSTR, IPVT, RCOND, WK )

      IF( 1.0 + RCOND.EQ.1.0 )
     &   CALL ERRMSG2('UPBEAM--SGECO says matrix near singular',.FALSE.)
...
c                ** ZJ;  return solution(s) in ZJ
      ...
      CALL SGESL2( ARRAY, MXCMU, NSTR, IPVT, ZJ, JOB )
   ...
   40 CONTINUE
      ...
      SUBROUTINE UPISOT2( ARRAY, CC, CMU, IPVT, MXCMU, NN, NSTR, OPRIM,
...
c                       ** except ZJ replaced by Z1 and Z0
      ...
      IF( 1.0 + RCOND.EQ.1.0 )
     &   CALL ERRMSG2('UPISOT--SGECO says matrix near singular',.False.)

      CALL SGESL2( ARRAY, MXCMU, NSTR, IPVT, Z1, 0 )

      DO 30 IQ = 1, NSTR
         Z0( IQ ) = ( 1. - OPRIM ) * XR0 + CMU( IQ ) * Z1( IQ )
   30 CONTINUE

      CALL SGESL2( ARRAY, MXCMU, NSTR, IPVT, Z0, 0 )
   ...
   40 CONTINUE
      ...
      SUBROUTINE USRINT2( BPLANK, CMU, CWT, DELM0, DTAUCP, EMU, EXPBEA,
     &                   FBEAM, FISOT, GC, GU, KK, LAMBER, LAYRU, LL,
     &                   LYRCUT, MAZIM, MXCMU, MXULV, MXUMU, NCUT, NLYR,
     &                   NN, NSTR, PLANK, NUMU, NTAU, PI, RMU, TAUCPR,
     &                   TPLANK, UMU, UMU0, UTAUPR, WK, ZBEAM, Z0U, Z1U,
...
c                            ** component for isotropic surface
  ...
  160 CONTINUE
      ...
      REAL FUNCTION  XIFUNC2( UMU1, UMU2, UMU3, TAU )
...
c     ..
      ...
      IF( UMU2.EQ.UMU3 .AND. UMU1.EQ.UMU2 ) THEN
         ...
         XIFUNC2 = TAU*TAU * EXP1 / ( 2.*UMU1*UMU2 )

      ELSE IF( UMU2.EQ.UMU3 .AND. UMU1.NE.UMU2 ) THEN

         XIFUNC2 = ( ( TAU - 1./X1 ) * EXP( -TAU/UMU2 ) + EXP1 / X1 )
     &            / ( X1*UMU1*UMU2 )

      ELSE IF( UMU2.NE.UMU3 .AND. UMU1.EQ.UMU2 ) THEN

         XIFUNC2 = ( ( EXP( -TAU/UMU3 ) - EXP1 ) / X2 - TAU * EXP1 )
     &            / ( X2*UMU1*UMU2 )

      ELSE IF( UMU2.NE.UMU3 .AND. UMU1.EQ.UMU3 ) THEN

         XIFUNC2 = ( ( EXP( -TAU/UMU2 ) - EXP1 ) / X1 - TAU * EXP1 )
     &            / ( X1*UMU1*UMU2 )

      ELSE

         XIFUNC2 = ( ( EXP( -TAU/UMU3 ) - EXP1 ) / X2 -
...
c ******************************************************************
      ...
      SUBROUTINE CHEKIN2(NLYR, DTAUC, SSALB, NMOM, PMOM, TEMPER, WVNMLO,
     &                   WVNMHI, USRTAU, NTAU, UTAU, NSTR, USRANG,
     &                   NUMU, UMU, NPHI, PHI, IBCND, FBEAM, UMU0,
     &                   PHI0, FISOT, LAMBER, ALBEDO, BTEMP, TTEMP,
     &                   TEMIS, PLANK, ONLYFL, DELTAM, CORINT, ACCUR,
...
c     .. External Functions ..
      ...
      REAL      DREF2
      EXTERNAL  WRTBAD2, WRTDIM2, DREF2
c     ..
c     .. External Subroutines ..
...
c     ..
      ...
      IF( NSTR.LT.2 .OR. MOD( NSTR,2 ).NE.0 ) INPERR = WRTBAD2( 'NSTR' )

      IF( NSTR.EQ.2 )
     &    CALL ERRMSG2( 'CHEKIN--2 streams not recommended; '//
   ...
   10 CONTINUE
      ...
      IF (NMOM.LT.NSTR) THEN
        write(0,*) '****  Input variable  NMOM  in error  ****'
        write(0,*) '      number of supplied phase',
     &             ' function Legendre coefficients ',NMOM
        write(0,*) '      is less than the number of streams ',NSTR
      ENDIF

      IF( NMOM.LT.0 .OR. ( YESSCT.GT.0.0 .AND. NMOM.LT.NSTR ) )
     &    INPERR = WRTBAD2( 'NMOM' )

      IF( MAXMOM.LT.NMOM ) INPERR = WRTBAD2( 'MAXMOM' )
   ...
   30 CONTINUE
      ...
      IF( IBCND.EQ.1 ) THEN
         ...
         IF( MAXULV.LT.2 ) INPERR = WRTBAD2( 'MAXULV' )

      ELSE IF( USRTAU ) THEN

         IF( NTAU.LT.1 ) INPERR = WRTBAD2( 'NTAU' )

         IF( MAXULV.LT.NTAU ) INPERR = WRTBAD2( 'MAXULV' )
   ...
   40    CONTINUE
      ...
      ELSE
         ...
         IF( MAXULV.LT.NLYR + 1 ) INPERR = WRTBAD2( 'MAXULV' )
   ...
   50    CONTINUE
      ...
      ELSE
         ...
         IF( MAXUMU.LT.NSTR ) INPERR = WRTBAD2( 'MAXUMU' )
   ...
   60    CONTINUE
      ...
      IF( IBCND.LT.0 .OR. IBCND.GT.1 ) INPERR = WRTBAD2( 'IBCND' )

      IF( IBCND.EQ.0 ) THEN

         IF( FBEAM.LT.0.0 ) INPERR = WRTBAD2( 'FBEAM' )
     ...
     &       INPERR = WRTBAD2( 'PHI0' )
         ...
         IF( FISOT.LT.0.0 ) INPERR = WRTBAD2( 'FISOT' )
...
c                    ** angles does not assume unphysical values
   ...
   70       CONTINUE
     ...
     &       INPERR = WRTBAD2( 'WVNMLO,HI' )
      ...
      IF( ACCUR.LT.0.0 .OR. ACCUR.GT.1.E-2 ) INPERR = WRTBAD2( 'ACCUR' )
     ...
     &       INPERR = WRTDIM2( 'MXULV', NLYR + 1 )

      ELSE

         IF( MXULV.LT.2 ) INPERR = WRTDIM2( 'MXULV', 2 )

      END IF

      IF( MXCMU.LT.NSTR ) INPERR = WRTDIM2( 'MXCMU', NSTR )

      IF( USRANG .AND. MXUMU.LT.NUMU ) INPERR = WRTDIM2( 'MXUMU', NUMU )

      IF( USRANG .AND. IBCND.EQ.1 .AND. MXUMU.LT.2*NUMU )
     &    INPERR = WRTDIM2( 'MXUMU', 2*NUMU )

      IF( .NOT.USRANG .AND. MXUMU.LT.NSTR )
     &    INPERR = WRTDIM2( 'MXUMU', NSTR )

      IF( .NOT.ONLYFL .AND. IBCND.NE.1 .AND. MXPHI.LT.NPHI )
     &    INPERR = WRTDIM2( 'MXPHI', NPHI )

      NUMSQT = 2*MAX( 100, NSTR )
      IF( MXSQT.LT.NUMSQT ) INPERR = WRTDIM2( 'MXSQT', NUMSQT )

      IF( INPERR )
     &   CALL ERRMSG2( 'DISORT--input and/or dimension errors', .True. )

      IF( PLANK .AND. quiet .eq. 0 ) THEN

         DO 80 LC = 1, NLYR

            IF( ABS( TEMPER( LC )-TEMPER( LC-1 ) ) .GT. 10.0 .AND. 
     &           TEMPSTEPWARN) THEN
               CALL ERRMSG2('CHEKIN--vertical temperature step larger'
     &           //' than 10K may be too large for good accuracy',
     &           .False. )
               TEMPSTEPWARN = .FALSE.
            ENDIF
   ...
   80    CONTINUE
     ...
     &     CALL ERRMSG2( 'CHEKIN--intensity correction is off; '//
     &                  'intensities may be less accurate', .False. )
      ...
      REAL FUNCTION  DREF2( WVNMLO, WVNMHI, MU, 
     &                      btype, brho0, bk, btheta, bsigma, bt1, bt2,
     &                      bscale, biso, bvol, bgeo,
     &                      bu10, bpcl, bsal, callnum )
...
c            btype   BRDF type 
c
c            brho0, bk, btheta, bscale   RPV BRDF parameterization 
c                                        (BM, 23.2.2001)
c
c            bu10   Cox and Munk water parameterization
c
c            bpcl   Pigment concentration for Cox and Munk (in mg.m-3)
c
c            bsal   Salinity for Cox and Munk parameterization (in ppt)
...
c     .. Scalar Arguments ..
      ...
      INTEGER   btype
      REAL      brho0, bk, btheta, bsigma, bt1, bt2
      REAL      bscale, biso, bvol, bgeo 
      REAL      bu10, bpcl, bsal
      INTEGER   callnum
c     ..
c     .. Local Scalars ..
...
c     ..
   ...
   10    CONTINUE
      ...
      IF( ABS( MU ).GT.1.0 )
     &    CALL ERRMSG2( 'DREF--input argument error(s)',.True. )
...
c                       ** Loop over angle of reflection
     ...
     &             btype, brho0, bk, btheta, bsigma, bt1, bt2, bscale,
     &             biso, bvol, bgeo, bu10, bpcl, bsal, callnum )
   20    CONTINUE

         DREF2 = DREF2 + GWT( JG )*SUM

   30 CONTINUE

      IF( DREF2.LT.0.0 .OR. DREF2.GT.1.0 ) 
     &     CALL ERRMSG2( 'DREF--albedo value not in (0,1)',.False. )
      
      RETURN
      END

      SUBROUTINE LEPOLY2( NMU, M, MAXMU, TWONM1, MU, SQT, YLM )
...
c                                   ** STWL(58a)
   ...
   70    CONTINUE
      ...
      REAL FUNCTION PLKAVG2( WNUMLO, WNUMHI, T )
...
c     ..
     ...
     &    CALL ERRMSG2('PLKAVG--temperature or wavenums. wrong',.TRUE.)
...
c                          ** by iterating Simpson rule to convergence.
   ...
   20    CONTINUE
         ...
         CALL ERRMSG2( 'PLKAVG--Simpson rule didnt converge',.FALSE.)

   30    CONTINUE

         PLKAVG2 = SIGDPI * T**4 * CONC * VAL
...
c                                    ** WNUMLO and WNUMHI both large
      ...
      PLKAVG2 = SIGDPI * T**4 * PLKAVG2

      IF( PLKAVG2.EQ.0.0 )
     &    CALL ERRMSG2('PLKAVG--returns zero; possible underflow',
     &    .FALSE.)
      ...
      SUBROUTINE PRAVIN2( UMU, NUMU, MAXUMU, UTAU, NTAU, U0U )
...
c     .. Array Arguments ..
      ...
      REAL      U0U( MAXUMU, * ), UMU( NUMU ), UTAU( NTAU )
c     ..
c     .. Local Scalars ..
...
c     ..
   ...
   20 CONTINUE
      ...
      SUBROUTINE PRTINP2(NLYR, DTAUC, DTAUCP, SSALB, NMOM, PMOM, TEMPER,
     &                   WVNMLO, WVNMHI, NTAU, UTAU, NSTR, NUMU, UMU,
     &                   NPHI, PHI, IBCND, FBEAM, UMU0, PHI0, FISOT,
     &                   LAMBER, ALBEDO, BTEMP, TTEMP, TEMIS, DELTAM,
     &                   PLANK, ONLYFL, CORINT, ACCUR, FLYR, LYRCUT,
...
c                                       ** two WRITEs above)
   ...
   20    CONTINUE
      ...
      SUBROUTINE PRTINT2( UU, UTAU, NTAU, UMU, NUMU, PHI, NPHI, MAXULV,
     &                   MAXUMU )
...
c     ..
     ...
     &          TWO / 2.D0 /
      ...
      IF( M.LT.1 ) CALL ERRMSG2( 'QGAUSN--Bad value of M',.True.)
...
c                                              ** Check for convergence
         IF( ABS( XI - X ).GT.TOL ) THEN
            ...
            IF( ITER.GT.MAXIT )
     &          CALL ERRMSG2( 'QGAUSN--max iteration count',.True.)
...
c                      ** from A*B because A*B may (over/under)flow
     ...
     &       ( A.LT.0.0 .AND. B.GT.0.0 ) ) RATIO2 = -RATIO2
      ...
      SUBROUTINE SLFTST2( CORINT, ACCUR, ALBEDO, BTEMP, DELTAM, DTAUC,
     &                   FBEAM, FISOT, IBCND, LAMBER, NLYR, PLANK, NPHI,
...
c                                    ** correct answers and abort if bad
         ...
         IF( ABS( ERROR1 ).GT.ACC ) OK = TSTBAD2( 'UU', ERROR1 )

         IF( ABS( ERROR2 ).GT.ACC ) OK = TSTBAD2( 'RFLDIR', ERROR2 )

         IF( ABS( ERROR3 ).GT.ACC ) OK = TSTBAD2( 'RFLDN', ERROR3 )

         IF( ABS( ERROR4 ).GT.ACC ) OK = TSTBAD2( 'FLUP', ERROR4 )

         IF( .NOT.OK ) CALL ERRMSG2('DISORT--self-test failed', .True. )
...
c                                      ** Restore user input values
   ...
   80    CONTINUE
      ...
      SUBROUTINE ZEROAL2( ND1, EXPBEA, FLYR, OPRIM, PHASA, PHAST, PHASM,
     &                        TAUCPR, XR0, XR1,
...
c ******************************************************************
      ...
      SUBROUTINE ALBTRN2( ALBEDO, AMB, APB, ARRAY, B, BDR, CBAND, CC,
...
c     .. Array Arguments ..
     ...
     &          TRNMED( MAXUMU ), U0U( MAXUMU, * ), UMU( MAXUMU ),
...
c     .. External Subroutines ..
      ...
      EXTERNAL  ALTRIN2, ERRMSG2, LEPOLY2, PRALTR2, SETMTX2, SGBCO2, 
     &          SOLEIG2, SOLVE12, SPALTR2, TERPEV2, ZEROIT2
c     ..
c     .. Intrinsic Functions ..
...
c                                       ** in Eq. STWJ(8b), STWL(23f)
         ...
         CALL SOLEIG2( AMB, APB, ARRAY, CMU, CWT, GL( 0,LC ), MI, MAZIM,
...
c                      ** LU-decompose the coeff. matrix (LINPACK)
      ...
      CALL SGBCO2( CBAND, MI9M2, NCOL, NCD, NCD, IPVT, RCOND, Z )
      IF( 1.0+RCOND .EQ. 1.0 )
     &   CALL ERRMSG2('ALBTRN--SGBCO says matrix near singular',.FALSE.)
...
c                             ** homogeneous solution
      ...
      CALL SOLVE12( B, CBAND, FISOT, 1, IPVT, LL, MI9M2, MXCMU,
...
c                             ** layer (Eqs. 3-4 of Ref S2)
      ...
      CALL ALTRIN2( GU, KK, LL, MXCMU, MXUMU, MAXUMU, NLYR, NN, NSTR,
...
c                             ** (if multiple layers)
         ...
         CALL SOLVE12( B, CBAND, FISOT, 2, IPVT, LL, MI9M2, MXCMU,
     &                NCOL, NLYR, NN, NNLYRI, NSTR )
...
c                             ** Get spherical albedo and transmissivity
         IF( NLYR.EQ.1 ) THEN
            ...
            CALL SPALTR2( CMU, CWT, GC, KK, LL, MXCMU, NLYR,
...
c                          ** agree with ordering in ALBMED, TRNMED
   ...
   80 CONTINUE
      ...
      IF( PRNT(4) ) CALL PRALTR2( UMU, NUMU, ALBMED, TRNMED )


      RETURN
      END

      SUBROUTINE ALTRIN2( GU, KK, LL, MXCMU, MXUMU, MAXUMU, NLYR, NN,
...
c                                        ** KK is positive
   ...
   50 CONTINUE
      ...
      SUBROUTINE PRALTR2( UMU, NUMU, ALBMED, TRNMED )
...
c     ..
   ...
   10 CONTINUE
      ...
      SUBROUTINE SOLVE12( B, CBAND, FISOT, IHOM, IPVT, LL, MI9M2, MXCMU,
...
c                             ** sources, remainder of B array is zero
   ...
   40 CONTINUE
      ...
      SUBROUTINE SPALTR2( CMU, CWT, GC, KK, LL, MXCMU, NLYR, NN, NSTR,
     &                   TAUCPR, SFLUP, SFLDN )
...
c ******************************************************************
      ...
      INTEGER FUNCTION LOCATE (N, XX, X)
c/* Given an array xx[1..n], and given a value x, returns a value j such that x is between xx[j] */
c/* and xx[j+1]. xx must be monotonic, either increasing or decreasing. j=0 or j=n is returned   */
c/* to indicate that x is out of range. A unit-offset array xx is assumed. To use locate with    */
c/* a zero-offset array, remember to subtract 1 from the address of xx, and also                 */
c/* from the returned value j.                                                                   */
c/*                                                                                              */
c/* From "Numerical recipes in C"                                                                */
c/* Translated to Fortran 77 by Robert Buras */
      ...
      DO WHILE ( JU - JL .GT. 1 )   !/* If we are not yet done,             */
         JM = (JU+JL)/2             !/* compute a midpoint,                 */
c         WRITE (*,*) 'locate', JL,JM,JU,XX(JM),X
         IF ( X .GE. XX( JM ) ) THEN
...
c      WRITE (*,*) 'locate end', JL,X,XX(1),XX(N)
      IF ( X .EQ. XX( 1 ) ) THEN
         LOCATE = 1                      !/* Then set the output */
      ...
      INTEGER FUNCTION DLOCATE (N, XX, X)
c/* Given an array xx[1..n], and given a value x, returns a value j such that x is between xx[j] */
c/* and xx[j+1]. xx must be monotonic, either increasing or decreasing. j=0 or j=n is returned   */
c/* to indicate that x is out of range. A unit-offset array xx is assumed. To use locate with    */
c/* a zero-offset array, remember to subtract 1 from the address of xx, and also                 */
...
c      SUBROUTINE CALC_PHASES (MAXMOM, MAXCLY, NPHAS, PMOM, PHASO, MUP,
c     &     PI)
c
c      INTEGER   MAXMOM, MAXCLY, NPHAS
c      REAL      PMOM( 0:MAXMOM, MAXCLY ), PI
c      REAL*8    PHASO( NPHAS, MAXCLY ), MUP( NPHAS )
c      INTEGER   LC, K, IT
c      REAL      CTHETA, PL, PLM1, PLM2, MUACT
c
c      MUACT = 0.
c      MUP ( NPHAS ) = MUACT
c
...
cc         WRITE ( *, '(A,F24.14)' )
cc     &        ' THETA =', MUP( IT )
c         MUP ( IT ) = COS ( MUP ( IT ) / 180.0 * PI )
cc         WRITE ( *, '(A,F24.14)' )
cc     &        ' MUP =', MUP( IT )
c 907  CONTINUE
c
c      DO 401 LC = 1, MAXCLY
c         DO 402 IT = 1, NPHAS
c            PHASO( IT, LC ) = 1.
c 402     CONTINUE
c 401  CONTINUE
c
cc      DO 444 K = 1, MAXMOM
cc         DO 445 LC = 1, MAXCLY
cc            WRITE (*,*) "mom",K,LC,PMOM(K,LC)
cc 445     CONTINUE
cc 444  CONTINUE
c
c      DO 403 IT = 1, NPHAS
...
c         DO 404 K = 1, MAXMOM
c
c            PL   = ( ( 2 *K-1 )*CTHETA*PLM1 - ( K-1 )*PLM2 ) / K
c            PLM2 = PLM1
c            PLM1 = PL
c
c            DO 405 LC = 1, MAXCLY
c               PHASO( IT, LC ) = PHASO( IT, LC ) + ( 2*K + 1 ) * PL *
c     &              PMOM( K, LC )
c 405        CONTINUE
c
cc            IF (IT .EQ. 99)
cc     &           WRITE (*,*) "PHASP",K,PHASO(IT,1)
c 404     CONTINUE
c
cc         DO 408 LC = 1, MAXCLY
cc            WRITE (*,*) "phaso",IT,LC,MUP(IT),PHASO( IT, LC )
cc 408     CONTINUE
c
c 403  CONTINUE
c
