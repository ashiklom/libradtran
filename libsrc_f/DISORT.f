c
c Changed as follow by A. Kylling (arve.kylling@nilu.no) 21.12.2000
c
...
c                     form required by LAPACK solution routines
c
...
c
c   IPVT(LC*IQ)       Integer vector of pivot indices for LAPACK
c                     routines
c
...
c                     (not saved as function of superscipt M)
c
c   Z()               scratch array used in ALBTRN to solve a linear 
c                     system for the constants of integration
c
...
c                                  ** Eq.SS(18) for incident beam source
     ...
     &                      IPVT, MAZIM, MXCMU, NN, NSTR, PI, UMU0,
...
c        special banded-matrix format required by LAPACK routines
c
c
c    I N P U T      V A R I A B L E S:
c
...
c   BAND STORAGE
c
c      LAPACK requires band matrices to be input in a special
...
c      will convert it to the form (ABD) required by LAPACK 
...
c
c       B        :  Right-hand side vector of Eq. SC(5) going into
c                   LAPACK band matrix solver; returns as solution 
c                   vector of Eq. SC(12), constants of integration 
c                   without exponential term
c
...
c              ** Solve linear system with banded coeff matrix CBAND
c              ** and right-hand side(s) B.  Solution is returned in B.
c              ** CBAND must be in LAPACK band matrix format, where the
c              ** diagonals become rows while keeping all elements in
c              ** their original columns.  (The LAPACK and LINPACK band
c              ** matrix formats are identical.)

      NCD = 3*NN - 1
      CALL SGBSV( NCOL, NCD, NCD, 1, CBAND, MI9M2, IPVT, B,NNLYRI, INFO)

      IF( INFO .GT. 0 )
     &    CALL ERRMSG('SOLVE0--SGBSV says matrix singular', .TRUE.)
      IF( INFO .LT. 0 )
     &    CALL ERRMSG('SOLVE0--bad argument to SGBSV', .TRUE.)

c                   ** Zero CBAND (it may contain 'foreign'
c                   ** elements upon returning from LAPACK);
...
c                                   ** Thermal source terms, STWJ(27c)
   ...
   80    CONTINUE
     ...
     &                   MXCMU, NN, NSTR, PI, UMU0, YLM0, YLMC, ZJ, ZZ )
...
c     .. Array Arguments ..
     ...
     &          GL( 0:MXCMU ), YLM0( 0:MXCMU ),
     &          YLMC( 0:MXCMU, * ), ZJ( MXCMU ), ZZ( MXCMU )
c     ..
c     .. Local Scalars ..

      INTEGER   IQ, JQ, K, INFO
      REAL      SUM
c     ..
c     .. External Subroutines ..

      EXTERNAL  ERRMSG, SGESV
c     ..
...
c           ** and right-hand side(s) ZJ;  return solution(s) in ZJ
c           ** ARRAY is altered on return.

      CALL SGESV( NSTR, 1, ARRAY, MXCMU, IPVT, ZJ, MXCMU, INFO )
      IF( INFO .GT. 0 )
     &    CALL ERRMSG('UPBEAM--SGESV says matrix singular', .TRUE.)
      IF( INFO .LT. 0 )
     &    CALL ERRMSG('UPBEAM--bad argument to SGESV', .TRUE.)
...
c     .. Array Arguments ..
      ...
      INTEGER   IPVT( * )
      REAL      ARRAY( MXCMU, MXCMU ), CC( MXCMU, MXCMU ), CMU( MXCMU ),
     &          Z0( MXCMU ), Z1( MXCMU ), ZPLK0( MXCMU ), ZPLK1( MXCMU )
c     ..
c     .. Local Scalars ..
...
c           ** Solve linear system with coeff matrix ARRAY
c           ** and right-hand side(s) Z0;  return solution(s) in Z0
c           ** (same as in UPBEAM except for right-hand side)

      CALL SGESV( NSTR, 1, ARRAY, MXCMU, IPVT, Z0, MXCMU, INFO )
      IF( INFO .GT. 0 )
     &    CALL ERRMSG('UPISOT--SGESV says matrix singular', .TRUE.)
      IF( INFO .LT. 0 )
     &    CALL ERRMSG('UPISOT--bad argument to SGESV', .TRUE.)
   ...
   20 CONTINUE
      ...
      ELSE IF( USRTAU ) THEN
         ...
         DO 30 LU = 1, NTAU
            ...
            IF( ABS( UTAU( LU ) - TAUC( NLYR ) ).LE. 1.E-3 )
     &          UTAU( LU ) = TAUC( NLYR )
...
c     .. Local Scalars ..
      ...
      LOGICAL   LAMBER, LYRCUT
      INTEGER   INFO, IQ, IU, L, LC, MAZIM, NCD, NCOL, NCUT
      REAL      DELM0, FISOT, SGN, SPHALB, SPHTRN, TOPISO, BOTISO
c     ..
c     .. External Subroutines ..

      EXTERNAL  ALTRIN, LEPOLY, PRALTR, SETMTX, SGBTRF, SOLEIG, SOLVE1,
...
c                      ** LU-decompose the coeff. matrix (LAPACK)
      ...
      IF(INFO.LT.0) 
     &   CALL ERRMSG('ALBTRN--bad argument to SGBTRF', .TRUE.)
      IF(INFO.GT.0) 
     &   CALL ERRMSG('ALBTRN--SGBTRF says matrix singular', .TRUE.)
...
c                             ** homogeneous solution
      ...
      BOTISO = 0.0
      TOPISO = FISOT
      CALL SOLVE1( B, CBAND, TOPISO, BOTISO, IPVT, LL, MI9M2, MXCMU,
...
c     ..
   ...
   10 CONTINUE
      ...
      SUBROUTINE SOLVE1( B, CBAND, TOPISO, BOTISO, IPVT, LL, MI9M2, 
     &                   MXCMU, NCOL, NCUT, NN, NNLYRI, NSTR )
...
c
c       TOPISO   :  (Isotropic) illumination at top boundary
c       BOTISO   :  (Isotropic) illumination at bottom boundary
c
c       NCOL     :  Number of columns in CBAND
c
c       NN       :  Order of double-Gauss quadrature (NSTR/2)
...
c     .. Local Scalars ..
      ...
      INTEGER   I, INFO, IPNT, IQ, LC, NCD
c     ..
c     .. External Subroutines ..

      EXTERNAL  ERRMSG, SGBTRS, ZEROIT
c     ..
...
c                             ** Solve linear system using LAPACK
c                             ** assuming CBAND already LU-decomposed
      NCD  = 3*NN - 1
      CALL SGBTRS( 'N', NCOL, NCD, NCD, 1, CBAND, MI9M2, IPVT, 
     &              B, NNLYRI, INFO )
      IF( INFO.LT.0 ) 
     &   CALL ERRMSG('SOLVE1--bad argument to SGBTRS', .TRUE.)
