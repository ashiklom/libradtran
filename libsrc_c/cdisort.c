/************************************************************************
 * $Id: cdisort.c 3319 2017-12-12 13:49:00Z Claudia.Emde $
 ************************************************************************/
	      ...
	      disort_output *out)
{
  ...
  int
    ...
    iq,iu,j,kconv,l,lc,lev,lu,k,mazim,naz,ncol,ncos,ncut,nn;
  static int
    callnum=1;
  int
			      ...
			      int          quiet )
{
  ...
  case BRDF_HAPKE:
    ans = c_bidir_reflectivity_hapke ( wvnmlo, wvnmhi, mu, mup, dphi,
				       brdf->hapke->b0, brdf->hapke->h, brdf->hapke->w );
    break;
  case BRDF_RPV:
    if ( swvnmlo != wvnmlo      ||
	 swvnmhi != wvnmhi      ||
  ...
  case BRDF_ROSSLI:
    ...
    if ( ans != ans ) {
      if (!quiet)
	fprintf(stderr,"NaN returned from brdf_rossli: %e %e %e %e %e %e %e %e\n",
		wvnmlo, wvnmhi, mu, mup, dphi, brdf->rossli->iso, brdf->rossli->vol, brdf->rossli->geo);
      ans = 1.0;
    }
...
double c_bidir_reflectivity_rossli ( rossli_brdf_spec *brdf,
				     double            mu,
				     double            mup,
				     double            dphi )
{
  double
    alpha0, ratio_hb, ratio_br, sin_i, sin_r, tan_i, tan_r, 
    cos_alpha, sin_alpha, alpha, c, f_vol,
    tan_i1, tan_r1, sin_i1, sin_r1, cos_i1, cos_r1,
    cos_alpha1, g_sq, cos_t, t, f_geo, ans;
  mu=fabs(mu);
  mup=fabs(mup);
  ...
  f_geo = (cos_i1+cos_r1)/(M_PI*cos_i1*cos_r1)*(t-sin(t)*cos(t)-M_PI) + (1.+ cos_alpha1)/(2.*cos_i1*cos_r1);
    ...
    for (iq = 1; iq <= nn; iq++) {
      for (jq = 1; jq <= nn; jq++) {
	    ...
	    c_bidir_reflectivity ( ds->wvnmlo, ds->wvnmhi, CMU(iq), CMU(jq),
				   M_PI * GMU(k), ds->flag.brdf_type, &ds->brdf, callnum, 0 )
      ...
      for (iq = 1; iq <= nn; iq++) {
        ...
        for (jg = 1; jg <= NMUG; jg++) {
	      ...
	      c_bidir_reflectivity ( ds->wvnmlo, ds->wvnmhi, CMU(iq), GMU(k),
				     M_PI * GMU(jg), ds->flag.brdf_type, &ds->brdf, callnum, 0 );
          }
          dref += GWT(jg)*sum;
        }
        BEM(iq) = 1.-dref;
      ...
      if (UMU(iu) > 0.) {
		...
		* cos( (double)mazim * M_PI * GMU(k) );
		...
		c_bidir_reflectivity ( ds->wvnmlo, ds->wvnmhi, UMU(iu),
				       ...
				       ds->flag.brdf_type, &ds->brdf, callnum, 0 )
		* cos( (double)mazim * M_PI * GMU(k) );
            }
	      ...
	      int          callnum )
{
  ...
  for (jg = 1; jg <= NMUG; jg++) {
    ...
    for (k = 1; k <= NMUG/2; k++) {
      sum += GWT(k) * GMU(k) *
	c_bidir_reflectivity ( wvnmlo, wvnmhi, GMU(k), mu, M_PI*GMU(jg), brdf_type, brdf, callnum, 1 );
    }
    ans += GWT(jg)*sum;
  }
  if (ans < 0. || ans > 1.) {
